...
class AnimationController {
    ...
    constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, dpr: number, size: number, onProgress?: (progress: number) => void) {
        this.canvas = canvas
        this.ctx = ctx
        this.dpr = dpr
        this.size = size
...
// Add prop type for SpiralAnimation
interface SpiralAnimationProps {
  onProgress?: (progress: number) => void
}

// Update SpiralAnimation to accept onProgress and pass to AnimationController
export function SpiralAnimation({ onProgress }: SpiralAnimationProps) {
    ...
    useEffect(() => {
        ...
        ctx.scale(dpr, dpr)
        animationRef.current = new AnimationController(canvas, ctx, dpr, size, onProgress)
        return () => {
